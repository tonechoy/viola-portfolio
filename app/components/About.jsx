    ...
    id='about' className='w-full px-[12%] py-10 scroll-mt-20'>
      ...
      className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        <div className='w-64 sm:w-80 rounded-3xl max-w-none'>
          <Image src={assets.viola_profile_pic1} alt='viola' className='w-full rounded-3xl'/>
        </div>
        <motion.div 
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        ...
        className='flex-1'>
          ...
          className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl font-outfit'>
            {infoList.map(({icon, iconDark, title, description}, index, key)=>(
              <motion.li 
              whileHover={{ scale: 1.05 }}
              key={index}
              className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-light-hover hover:-translate-y-1 duration-300 hover:shadow-black dark:border-gray-200 dark:hover:bg-dark-hover/50 dark:hover:shadow-gray-200'>
                <Image src={isDarkMode ? iconDark : icon} alt={title} className='w-7 mt-3'/>
                <h3 className='my-4 font-semibold text-gray-700 dark:text-gray-200'>{title}</h3>
                <p className='text-gray-600 text-sm dark:text-gray-200/80'>{description}</p>
              </motion.li>
            ))}
          </motion.ul>
          ...
          className='my-6 text-gray-700 font-ovo dark:text-gray-300'>Tools I use</motion.h4>
          <motion.ul 
          initial={{opacity: 0}}
          whileInView={{opacity: 1}}
          transition={{duration: 1.5, delay: 0.6}}
          className='flex items-center gap-3 sm:gap-5'>
            {toolsData.map((tool, index, key)=>(
              <motion.li 
